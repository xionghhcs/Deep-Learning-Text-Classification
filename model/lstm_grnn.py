...
class LSTM_GRNN(BasicModel):
    ...
    def _sentence_composition(self, text_embed):
        ...
        for idx, s in enumerate(sentences):
            with tf.variable_scope('sentence_compositon_{}'.format(idx)):
                fw_cell = tf.nn.rnn_cell.BasicLSTMCell(self.sent_rnn_units)
                bw_cell = tf.nn.rnn_cell.BasicLSTMCell(self.sent_rnn_units)
                ...
                composition_result.append(output)
    ...
    def _document_composition(self, sentence_composition):
        fw_cell = tf.nn.rnn_cell.BasicLSTMCell(self.doc_rnn_units)
        bw_cell = tf.nn.rnn_cell.BasicLSTMCell(self.doc_rnn_units)

        output, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=fw_cell, cell_bw=bw_cell, inputs=sentence_composition,
                                                    dtype=tf.float32)
        output = tf.concat(output, axis=-1)

        doc_composition = tf.reduce_max(output, axis=1)
