...
class Char_Level_ConvNet(BasicModel):
    def __init__(self, maxlen=1024, num_classes=2, num_chars=71, embed_dim=16, kernel_sizes=[7, 7, 3, 3, 3, 3],
                 lr=0.001, model_path='../tmp/char_level_conv_net'):
        super(Char_Level_ConvNet, self).__init__()
    ...
    def build_model(self):
        with tf.variable_scope('inputs'):
            ...
            self.dense_dropout_input = tf.placeholder_with_default(0.0, shape=(), name='dense_dropout_input')
        ...
        with tf.variable_scope('cnn_layers'):
            ...
            for idx, kernel_size in enumerate(self.kernel_sizes):
                with tf.variable_scope('conv_layer_{}'.format(idx)):
                    x = tf.layers.conv1d(x, filters=256, kernel_size=kernel_size)
                    if idx < 2:
                        # pooling is non-over-lapping, so the stride should be the same as pool_size
