...
class FastText(BasicModel):
    def __init__(self, matrix=None, maxlen=None, num_classes=2, embed_dropout=0.0, dense_dropout=0.4, lr=0.001,
                 model_path='../tmp/text_cnn'):
    ...
    def build_model(self):
        ...
        with tf.variable_scope('embedding_average'):
            text_rep = tf.reduce_mean(text_embed, axis=1)

        with tf.variable_scope('output_layer'):
            dense = tf.concat(text_rep, axis=-1)
            dense = tf.layers.dropout(dense, rate=self.dense_dropout, training=self.train_flag)
