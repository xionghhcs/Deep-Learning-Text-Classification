...
class RCNN(BasicModel):
    def __init__(self, matrix=None, maxlen=None, num_classes=2, rnn_units=128, rnn_dropout=0.1,
                 embed_dropout=0.0, dense_dropout=0.4, lr=0.001, model_path='../tmp/rcnn'):
    ...
    def build_model(self):
        ...
        with tf.variable_scope('birnn_layer'):
            fw_cell = tf.nn.rnn_cell.BasicLSTMCell(self.rnn_units)
            fw_cell = tf.nn.rnn_cell.DropoutWrapper(fw_cell, output_keep_prob=1 - self.rnn_dropout)
            bw_cell = tf.nn.rnn_cell.BasicLSTMCell(self.rnn_units)
            bw_cell = tf.nn.rnn_cell.DropoutWrapper(bw_cell, output_keep_prob=1 - self.rnn_dropout)

            (fw_output, bw_output), _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=fw_cell, cell_bw=bw_cell,
                                                                        inputs=text_embed,
                                                                        dtype=tf.float32,
                                                                        sequence_length=RCNN._length(self.text_input))

        with tf.variable_scope('context'):
            zero_shape = [tf.shape(fw_output)[0], 1, tf.shape(fw_output)[2]]

            c_left = tf.concat([tf.zeros(zero_shape), fw_output[:, :-1]], axis=1)
            c_right = tf.concat([bw_output[:, 1:], tf.zeros(zero_shape)], axis=1)

        with tf.variable_scope('word_representation'):
            x = tf.concat([c_left, text_embed, c_right], axis=-1)
            y2 = tf.layers.dense(x, units=20, activation=tf.nn.tanh)
    ...
    @staticmethod
    def _length(seq):
        useful_place = tf.sign(tf.abs(seq))
        length = tf.reduce_sum(useful_place, axis=-1)
